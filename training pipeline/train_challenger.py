...
@task(name = "Read Data", retries = 4, retry_delay_seconds = [1,3])
...
@flow(name="Model Comparison and Alias Assignment")
def model_comparison_flow(year: str, month_train: str, month_val: str) -> None:
    ...
    # Train two models
    rmse1, run_id1 = train_model(X_train, X_val, y_train, y_val, params1, dv, run_name="model_1")
    rmse2, run_id2 = train_model(X_train, X_val, y_train, y_val, params2, dv, run_name="model_2")

    # Register models and assign aliases
    if rmse1 < rmse2:
        champion_version = register_model(run_id1, "nyc-taxi-model-prefect", "champion")
        challenger_version = register_model(run_id2, "nyc-taxi-model-prefect", "challenger")
    else:
        champion_version = register_model(run_id2, "nyc-taxi-model-prefect", "champion")
        challenger_version = register_model(run_id1, "nyc-taxi-model-prefect", "challenger")
